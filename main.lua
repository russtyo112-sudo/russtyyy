-- XenoExecutor — FRONTLINES edition
-- Fixes: Rthro rig detection, custom health, broader part search,
--        looser team check, deeper character scanning
...
-- Rthro + custom rigs: parts can be deeply nested in Model folders
-- We recursively find all BaseParts and the Humanoid anywhere in char
local function getAllParts(model)
    ...
    local function scan(obj)
        for _, v in ipairs(obj:GetChildren()) do
            ...
            -- recurse into Models but not tools/held items
...
local function findHumanoid(model)
    -- check direct children first (standard), then recurse
...
local function findHead(model)
    -- try standard name first, then any part named Head anywhere
    local h = model:FindFirstChild("Head", true)
    if h and h:IsA("BasePart") then return h end
    -- Rthro fallback — HumanoidRootPart or UpperTorso
...
-- Check if player is alive (handles custom health values)
local function isAlive(plr)
    local char = plr.Character
    if not char then return false end
    local hum = findHumanoid(char)
    if not hum then return false end
    -- some games use MaxHealth of 0 as "dead" or custom values
    if hum.Health <= 0 then return false end
    -- extra check: HumanoidRootPart must exist and not be at 0,0,0
    local root = findRootPart(char)
    if not root then return false end
    return true
end

-- Check if two players are on the same team (robust for Frontlines)
...
local function createESPFor(plr)
    ...
    edge(box).Size = UDim2.new(1,0,0,1); box:FindFirstChild("Frame").Position = UDim2.new(0,0,0,0)
    local top = box:FindFirstChild("Frame")
    top.Size = UDim2.new(1,0,0,1); top.Position = UDim2.new(0,0,0,0)
...
-- Bounding box using recursive part search
...
local function getAimPart(char)
    ...
    -- body or head fallback: UpperTorso → HumanoidRootPart
...
reopenBtn.MouseButton1Click:Connect(function()
    mainPanel.Visible = true; reopenBtn.Visible = false
end)
...
closeBtn.MouseButton1Click:Connect(function()
    mainPanel.Visible = false; reopenBtn.Visible = true
end)
...
local function mkAimLevel(y)
    ...
    headBtn.TextColor3 = Color3.fromRGB(255,255,255); headBtn.Text = "HEAD"
    headBtn.Size = UDim2.new(0.5,-14,0,30); headBtn.Position = UDim2.new(0,10,0,y)
    ...
    bodyBtn.TextColor3 = Color3.fromRGB(255,255,255); bodyBtn.Text = "BODY"
    bodyBtn.Size = UDim2.new(0.5,-14,0,30); bodyBtn.Position = UDim2.new(0.5,4,0,y)
